...
export function ProdutoForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setName("");
      showToast("success", "Produto cadastrado para movimentação.");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-wrap items-end gap-2">
      <div className="flex min-w-[min(100%,280px)] flex-1 flex-col gap-1">
        <label htmlFor="prod" className="text-xs font-medium text-zinc-600">
          Novo produto (nome único)
        </label>
        <input
          id="prod"
          value={name}
          ...
          placeholder="Ex.: nome usado na NF e no barracão"
          required
        />
      </div>
      <button
